        ...
        if not env.is_terminal(state):
            ...
            for action in env.actions:
                ...
                if value > best_value:
                    best_value = value
                    best_action = action
            new_policy[state] = {action: 1 if action == best_action else 0 for action in env.actions}
...
def PolicyIteration(env: GridWorld, theta: float):
    ...
    """
    policy = {state: {action: 1 / len(env.actions) for action in env.actions} for state in env.all_states}
    while True:
...
def ValueIteration(env: GridWorld, theta: float) -> np.ndarray:
    ...
    for state in env.all_states:
        if not env.is_terminal(state):
            best_action = max(env.actions, key=lambda action: env.P[state][action][1] + env.gamma * V[env.P[state][action][0]])
